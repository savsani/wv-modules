...
<div @if($error || $success) class="relative" @endif>
    <input
        type="{{ $type }}"
        ...
        {{ $attributes->merge(['class' => "form-input {$stateClass} h-11"]) }}
    />
    @if($error)
        <span class="pointer-events-none absolute top-1/2 right-3.5 -translate-y-1/2 text-red-500 dark:text-red-400">
    ...
    @elseif($success)
        <span class="pointer-events-none absolute top-1/2 right-3.5 -translate-y-1/2 text-green-500 dark:text-green-400">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
